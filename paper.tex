...
written in far less space.  This property is especially helpful for conferences
and journals with a hard limit on the number of pages including the bibliography
(including many with a printed publication).  For example, the 274 authors of
the human genome project \cite{human-genome} would normally take almost half
a page to list:
...
As a result, many papers that cite this paper do not even list the entire
author list in the bibliography, instead writing ``J. C. Venter et al.''
...
giving credit to all authors without allocating that paper a disproportionate
amount of space in the bibliography.
